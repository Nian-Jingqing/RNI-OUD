function data = CR_training_slider_triggers(subjectID, sessionID, protocolID, noExposure)
...
% Modified to incorporate new subfunctions showInstructScreens and
% showRatingSlider. Also started adding more functionality to the
% configuration parameters
%
% Script Dependencies:
% Config_CR.m, image_list_gen_training.m, preload_training_images.m
% showInstructionScreens.m, showRatingSlider.m
%
% Config Parameters:
...
try
    % Close all ports if any are still open
    IOPort('CloseAll');
    % Open the IOPort
...
save_name = [config.data, '/', subjectID '_' sessionID,'_', datestr(now,'mm_dd_yyyy__HH_MM_SS') '.mat'];
excel_name = [save_name(1:end-4), '_updating.csv'];
...
[list, folder_names] = image_list_gen_training(subjectID, sessionID, protocolID, noExposure);
...
for j = 1:numImgs
    ...
    triggerTimes{j} = sendTrigger(handle,255,0.05);
   
    % Allow the image to be on screen for x seconds
    WaitSecs(img_durations(j));

    % Draw the rating slider ----------------------------------------------
    [ratings{j}, RT{j}, resp{j}] = showRatingSlider(w,craving_scales,craving_scales_submitted,sliderStartPosition,sliderDuration,keys,textColor,base_rating);
    ...
    record = [record; {img_starts{j},triggerTimes{j},ratings{j}, RT{j}, resp{j}}];
    writecell(record,excel_name);
    % ---------------------------------------------------------------------
end
