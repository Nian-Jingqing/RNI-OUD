function [] = Copy_of_Delay_Discounting_5Trial(subjectID,sessionID,protocolID)
...
configPath = [basePath, 'DD Config Files'];
addpath(configPath);
...
saveName = [dataPath, subjectID, '/' subjectID, '_DD_5Trial_' sessionID,'_', datestr(now,'mm_dd_yyyy_HH_MM') '.mat'];

% Load the traing configuration struct
config = DD_5Trial_config;
...
for j = 1%:length(time_strings)
    % Restart the task at half eqValue
    tmp = eqValue/2;
...
%         DrawFormattedText(w, '+','center', 'center', textColor,[],0,0);
% 
%         % Collect timestamp of fixation flip
%         interTrialFixationStamps{j,i} = Screen('Flip', w);
%         WaitSecs(interTrialDuration);
